...
def test_find_conflicts(tmp_path):
    async def scenario():
        ...
        assert conflicts
        overlap_conflicts = await events_service.find_conflicts(
            creator_id=2,
            room="7А",
            starts_at=base + timedelta(minutes=30),
        )
        assert overlap_conflicts
        distant_conflicts = await events_service.find_conflicts(
            creator_id=2,
            room="7А",
            starts_at=base + timedelta(hours=2),
        )
        assert not distant_conflicts
