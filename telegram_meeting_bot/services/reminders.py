...
class ReminderService:
    ...
    async def _dispatch_chat_digest(self, now: datetime) -> None:
        ...
        for chat in chats:
            ...
            for event in events:
                local_time = to_local(event.starts_at, chat.timezone)
                lines.append(
                    f"• {local_time.strftime('%H:%M')} — {event.title} / {event.room} / {event.ticket}"
                )
            text = "\n".join(lines)
    ...
    async def _dispatch_user_digest(self, now: datetime) -> None:
        ...
        for user in users:
            ...
            for event in events:
                local_time = to_local(event.starts_at, user.timezone)
                lines.append(
                    f"• {local_time.strftime('%H:%M')} — {event.title} / {event.room} / {event.ticket}"
                )
            text = "\n".join(lines)
