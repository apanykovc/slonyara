...
class EventsService:
    ...
    async def _check_conflicts(
        ...
        *,
        ...
        draft_start = draft.starts_at
        draft_end = draft_start + self._default_duration
        tol = tolerance
        draft_room = (draft.room or "").strip().lower()
        for event in self._events.values():
            ...
            event_end = event_start + self._default_duration
            if draft_start > event_end - tol:
                continue
            if event_start > draft_end - tol:
                continue
            same_creator = event.creator_id == draft.creator_id
            event_room = (event.room or "").strip().lower()
            same_room = draft_room and event_room and event_room == draft_room
            if same_creator or same_room:
